...
from medg.data.utils import is_file_list
from medg.utils.writer import nifti_writer
from medg.utils.misc import rgetattr, rsetattr
from medg.transforms.defaults import DEFAULT_TRANSFORM_SETTINGS

BASE_TRANSFORMS = [
    tf.LoadImaged(keys=['image', 'label'], image_only=False),
    tf.EnsureChannelFirstd(keys=['image'], channel_dim=0),
    tf.EnsureChannelFirstd(keys=['label'], channel_dim='no_channel'),
]
...
class DatasetGenerator(object):
    ...
    def __init__(self, input_files, out_path, transform_settings=None):
        ...
        for i, sample_dict in enumerate(self.input_files):
            filename_mappings[sample_dict['image']] = str(i + 1).zfill(6)
    ...
    def generate_dataset(self):
        ...
        for transform_name, settings in self.transform_settings.items():
            ...
            for i in range(len(sv_controller[next(iter(sv_controller))])):
                ...
                print(transform_name, *[(param, rgetattr(transform, param)) for param in sv_controller.keys()])
                ...
                for j, test_data in enumerate(loader):
                    orig_file_path = Path(test_data['image_meta_dict']['filename_or_obj'][0])
                    orig_label_path = Path(test_data['label_meta_dict']['filename_or_obj'][0])

                    # subject_id = self.filename_mappings[test_data['label_meta_dict']['filename_or_obj'][0]]
                    affine = test_data['image_meta_dict']['affine'][0]
                    save_dir = os.path.join(self.out_path, f'{transform_name}_{i + 1}')
                    ...
                    for data, orig_path in zip([test_inputs, test_labels], [orig_file_path, orig_label_path]):
                        path_to_save_to = os.path.join(save_dir, orig_path.parent.name, orig_path.name)
