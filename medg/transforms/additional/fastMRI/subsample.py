...
class MaskFunc:
    ...
    def sample_mask(
        ...
        acceleration_mask = self.reshape_mask(
            self.calculate_acceleration_mask(num_cols, acceleration, num_low_frequencies, offset),
            shape,
        )
    ...
    def calculate_acceleration_mask(
            self,
            num_cols: int, acceleration: int, num_low_frequencies: int,
            offset: Optional[int] = 0
    ) -> np.ndarray:
        """
...
class RandomMaskFunc(MaskFunc):
    ...
    def calculate_acceleration_mask(self, num_cols: int, acceleration: int, num_low_frequencies: int,
...
class MagicMaskFractionFunc(MagicMaskFunc):
    ...
    def sample_mask(self, shape: Sequence[int], offset: Optional[int] = 0) -> Tuple[torch.Tensor, torch.Tensor, int]:
        """
        Sample a new k-space mask.
        ...
        if adjusted_acceleration == 0:
            accel_mask = torch.zeros_like(center_mask)
        else:
            accel_mask = self.reshape_mask(
                self.calculate_acceleration_mask(num_cols, adjusted_acceleration, num_low_frequencies, offset),
                shape,
            )
