...
import numpy as np
import torch
from torchio import IntensityTransform, FourierTransform, Subject
from torchio.transforms.augmentation import RandomTransform


class RandomSpikeN(RandomTransform, IntensityTransform, FourierTransform):
    ...
    def apply_transform(self, subject: Subject) -> Subject:
        ...
        transform = Spike(spatial_axis=self.spatial_axis, **self.add_include_exclude(arguments))
    ...
    def get_params(
        ...
        spikes_positions = torch.rand(num_spikes_param, self.spatial_axis).numpy()
...
class Spike(IntensityTransform, FourierTransform):
    ...
    def add_artifact(
        ...
        if self.spatial_axis == 2:
            spikes_positions = np.concatenate((spikes_positions, np.zeros((len(spikes_positions), 1))), axis=1)
            spectrum = torch.fft.fftshift(torch.fft.fftn(tensor, dim=(0, 1), norm="ortho"))
        ...
        if self.spatial_axis == 2:
            spectrum = torch.fft.ifftn(torch.fft.ifftshift(spectrum), dim=(0, 1), norm="ortho")
        else:
            spectrum = torch.fft.ifftn(torch.fft.ifftshift(spectrum), norm="ortho")

        result = spectrum.real.float()
